...
// Establecer la fecha de finalización al último día del mes actual
const endDate = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59); // Último día de agosto 2024, a las 23:59:59
...
function updateDateElement(cardElement, cardElementValue) {
  ...
  updateCardValues(cardElements.cardElementTop, cardElements.cardElementFlipBottom, cardElementValue);

  function endAnimation() {
    cardElements.cardElementFlip.classList.remove('anim');
    updateCardValues(cardElements.cardElementBottom, cardElements.cardElementFlipTop, cardElementValue);
    this.removeEventListener('animationend', endAnimation);
  }

  cardElements.cardElementFlip.addEventListener('animationend', endAnimation);
}
...
function getRemainingTime(target) {
  ...
  const secondsRemaining = Math.floor((target - currentTime) / 1000);
...
function updateAllElements() {
  const timeRemainingForUpdate = getRemainingTime(new Date(endDate).getTime());
